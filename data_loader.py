...
class DataLoader:
    ...
    def load_transform_file(self, file_path):
        ...
        # TODO: Your code here
        query="""LOAD CSV WITH HEADERS FROM "file:///%s.csv" AS row
MERGE (pickup:Location {name: toInteger(row.PULocationID)})
MERGE (dropoff:Location {name: toInteger(row.DOLocationID)})
CREATE (pickup)-[t:TRIP { distance: toFloat(row.trip_distance), fare: toFloat(row.fare_amount), pickup_dt: datetime(replace(row.tpep_pickup_datetime, ' ', 'T')), dropoff_dt: datetime(replace(row.tpep_dropoff_datetime, ' ', 'T')) }]->(dropoff)""" % file_path.split(".")[0]
